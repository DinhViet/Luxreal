...
public class MapUtil {
  ...
  public static Map<String, String> httpRequestParameterToMap(HttpServletRequest req) {
      ...
      try {
        ...
        if (StringUtils.isNotEmpty(name))
          map.put(URLDecoder.decode(name, "UTF-8"),
              nameValue.length > 1 ? URLDecoder.decode(value, "UTF-8") : "");
      } catch (UnsupportedEncodingException e) {
        throw new RuntimeException("This method requires UTF-8 encoding support", e);
      }
    }
  ...
  /*
   * public static void main(String[] args) { String input = "a="; queryStringToMap(input); }
   */
}
