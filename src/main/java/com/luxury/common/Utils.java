...
public class Utils {
	...
	public static Date changeDate(int dayOfMonth,Date oldDate) {
		Date newDate = null;
  ...
  /*public static Date getDateTimeZone7(Date date){
	  ...
	  try {
		  ...
		  currentDate = now.toLocalDateTime().toDate(); 
	  } catch (Exception e) {
			e.printStackTrace();
		}
	  return currentDate;
  ...
  public static String generateAccountCode(int number) {
  ...
  public static String[] mobileOperators = {"VNP", "VMS", "VTM", "SFE", "VNM", "BEL", "GATE",
      "VCOIN", "ZING", "UNKNOWN"};
  ...
  public static Date getDateFirtDay(String fromDate){
	  Date dateFrom =null;
  ...
  public static Date getDateLastDay(String toDate){
	  Date dateFrom =null;
	  SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy");
		try {
			dateFrom = df.parse(toDate);
			Calendar cal = Calendar.getInstance();
			cal.setTime(dateFrom);
  ...
  public static String getIP(HttpServletRequest req) {
    ...
    if (ip != null && (ip.startsWith("127.") || ip.startsWith("192.168."))) {
      ...
      String userAgent =
          req.getHeader("User-Agent") != null ? req.getHeader("User-Agent") : req
              .getHeader("user-agent");
      if (StringUtils.isNotEmpty(userAgent) && (userAgent.toLowerCase().indexOf("opera") != -1)
          && (userAgent.toLowerCase().indexOf("mini") != -1)) {
        ip =
            req.getHeader("HTTP_X_FORWARDED_FOR") != null ? req.getHeader("HTTP_X_FORWARDED_FOR")
                : req.getHeader("x-forwarded-for") != null ? req.getHeader("x-forwarded-for") : req
                    .getHeader("X-Forwarded-For");
      } else {
        ip =
            StringUtils.isNotEmpty(req.getHeader("vx-forwarded-for")) ? req
                .getHeader("vx-forwarded-for") : req.getHeader("x-forwarded-for") != null ? req
                .getHeader("x-forwarded-for") : req.getHeader("X-Forwarded-For");
      }
      if (ip != null) {
  ...
  public static boolean validateMsisdn(String msisdn){
	  String trimmedMsisdn = StringUtils.trimToEmpty(msisdn);
	  if(trimmedMsisdn.length() == 0)
		  return false;
	  for(int i = 0 ;i < trimmedMsisdn.length() ; i++)
		  if(!Character.isDigit(trimmedMsisdn.charAt(i)))
			  return false;
	  return true;
  }
